...
const uploadAndScanImage = asyncHandler(async (req, res) => {
   ...
   try {
    ...
    // Try to parse the AI output as JSON; fall back to a structured object
    let parsed = null;
    try {
        ...
        // If parsing fails, provide a concise fallback
        parsed = {
            disease: "Unknown",
            confidence: "0",
            description: text,
    ...
    // Optionally, you could persist the image/analysis. For now, return analysis.
    return res
        .status(200)
        .json(
            new ApiResponse(
